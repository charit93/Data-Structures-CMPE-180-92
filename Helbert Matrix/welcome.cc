 ...
 */
int HilbertMatrix(const int n,double A[][MAX_SIZE])
{
for(int i=1;i<=n; i++)    //This loops on the rows.
{
for(int j=1;j<=n; j++) //This loops on the columns
{
int sum;
double inverse;
sum=i+j-1;
...
int Multiply(const int n,double A[][MAX_SIZE],double Ainv[][MAX_SIZE])
{
double multiply[][MAX_SIZE];
cout.precision(3);
for (int i = 1; i <=n; i++)
{
for (int j = 1; j <=n; j++)
